...
/*            #if UNITY_EDITOR
        ...
        public void UpdateWaveMotion(Vector3 motionDirection, float strength)
        {
            for (int i = 0; i < _waves.Length; i++)
            {
                // Ví dụ: gán giá trị của motionDirection.x vào một biến float
                float xComponent = motionDirection.x;

                // Hoặc tính toán một giá trị float từ Vector3
                float computedValue = motionDirection.magnitude * strength;

                // Thực hiện các thao tác cần thiết với các giá trị float này
                // Ví dụ:
                _waves[i].amplitude = computedValue;
            }

            // Cập nhật lại thông số sóng vào shader
            UpdateWaveParameters();
        }


        // Hàm để cập nhật thông số sóng vào shader
        private void UpdateWaveParameters()
        {
            // Cập nhật các thông số sóng vào shader tương ứng
            if (_useComputeBuffer)
            {
                waveBuffer?.SetData(_waves); // Cập nhật dữ liệu sóng vào compute buffer
            }
            else
            {
                Shader.SetGlobalVectorArray(WaveData, GetWaveData()); // Cập nhật dữ liệu sóng trên CPU
            }

            // Các cập nhật khác nếu cần thiết
        }
    }
}
